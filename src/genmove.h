...
#define HistoryP(piece,from,to) ((int(HistoryScore(piece,from,to) & 0xFF00) / int(HistoryScore(piece,from,to) & 0x00FF)) << 16)
...
#define UpdateRef(ref_move) if ((Current->move) && RefM(Current->move).ref[0] != (ref_move)) { \
	RefM(Current->move).ref[1] = RefM(Current->move).ref[0]; RefM(Current->move).ref[0] = (ref_move); }
#define UpdateCheckRef(ref_move) if ((Current->move) && RefM(Current->move).check_ref[0] != (ref_move)) { \
