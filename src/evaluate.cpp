...
#define UpdateDelta                                                                                                    \
	if (!(Current->capture) && (Current->move != 0) && !(Current->move & 0xE000) && Current > Data) {                  \
...
void print_eval()
{
	...
	for (j = 0; j < 4; j++) {
		fprintf(f, "%d:\n", j);
		for (i = 0; i < 32; i++)
			fprintf(f, "(%d,%d), ", Opening(Mobility[j][i]), Endgame(Mobility[j][i]));
		fprintf(f, "\n");
	}
	fprintf(f, "PasserGeneral\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserGeneral[i]), Endgame(PasserGeneral[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserBlocked\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserBlocked[i]), Endgame(PasserBlocked[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserFree\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserFree[i]), Endgame(PasserFree[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserSupported\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserSupported[i]), Endgame(PasserSupported[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserProtected\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserProtected[i]), Endgame(PasserProtected[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserConnected\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserConnected[i]), Endgame(PasserConnected[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserOutside\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserOutside[i]), Endgame(PasserOutside[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserCandidate\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserCandidate[i]), Endgame(PasserCandidate[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserClear\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "(%d,%d), ", Opening(PasserClear[i]), Endgame(PasserClear[i]));
	fprintf(f, "\n");

	fprintf(f, "PasserAtt\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "%d, ", PasserAtt[i]);
	fprintf(f, "\n");

	fprintf(f, "PasserDef\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "%d, ", PasserDef[i]);
	fprintf(f, "\n");

	fprintf(f, "PasserAttLog\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "%d, ", PasserAttLog[i]);
	fprintf(f, "\n");

	fprintf(f, "PasserDefLog\n");
	for (i = 0; i < 8; i++)
		fprintf(f, "%d, ", PasserDefLog[i]);
	fprintf(f, "\n");

	fprintf(f, "StormBlocked\n");
	for (i = 0; i < 4; i++)
		fprintf(f, "%d, ", StormBlocked[i]);
	fprintf(f, "\n");

	fprintf(f, "StormShelterAtt\n");
	for (i = 0; i < 4; i++)
		fprintf(f, "%d, ", StormShelterAtt[i]);
	fprintf(f, "\n");

	fprintf(f, "StormConnected\n");
	for (i = 0; i < 4; i++)
		fprintf(f, "%d, ", StormConnected[i]);
	fprintf(f, "\n");

	fprintf(f, "StormOpen\n");
	for (i = 0; i < 4; i++)
		fprintf(f, "%d, ", StormOpen[i]);
	fprintf(f, "\n");

	fprintf(f, "StormFree\n");
	for (i = 0; i < 4; i++)
		fprintf(f, "%d, ", StormFree[i]);
	fprintf(f, "\n");
...
void init_eval()
{
	...
	for (i = 0; i < 4; i++) {
		StormBlocked[i] =
		    ((Sa(StormQuad, StormBlockedMul) * i * i) + (Sa(StormLinear, StormBlockedMul) * (i + 1))) / 100;
		StormShelterAtt[i] =
		    ((Sa(StormQuad, StormShelterAttMul) * i * i) + (Sa(StormLinear, StormShelterAttMul) * (i + 1))) / 100;
		StormConnected[i] =
		    ((Sa(StormQuad, StormConnectedMul) * i * i) + (Sa(StormLinear, StormConnectedMul) * (i + 1))) / 100;
		StormOpen[i] = ((Sa(StormQuad, StormOpenMul) * i * i) + (Sa(StormLinear, StormOpenMul) * (i + 1))) / 100;
		StormFree[i] = ((Sa(StormQuad, StormFreeMul) * i * i) + (Sa(StormLinear, StormFreeMul) * (i + 1))) / 100;
	}
...
template <bool me> int kpkx(const int turn, const GBoard *Board)
{
	...
	bitboard_t u;
	if (me == White)
		u = Kpk[turn][lsb(Pawn(White))][lsb(King(White))] & Bit(lsb(King(Black)));
	else
		u = Kpk[turn ^ 1][63 - lsb(Pawn(Black))][63 - lsb(King(Black))] & Bit(63 - lsb(King(White)));

	if (u != Empty)
		return 32;
	else if ((Piece(opp) ^ King(opp)) != Empty)
		return 1;
	else
		return 0;
}
...
{
	constexpr bool opp = !me;
	if ((Pawn(me) & Line(me, 6) & (File[0] | File[7])) != Empty) {
		int sq = lsb(Pawn(me));
		if ((SArea[sq] & King(opp) & (Line(me, 6) | Line(me, 7))) != Empty)
			return 0;
		if (Square(sq + Push(me)) == IKing(me) &&
		    (SArea[lsb(King(me))] != Empty && (SArea[lsb(King(opp))] & Line(me, 7)) != Empty))
			return 0;
	} else if ((Pawn(me) & Line(me, 5) & (File[0] | File[7])) != Empty) {
		int sq = lsb(Pawn(me));
		if (Square(sq + Push(me)) == IPawn(opp)) {
			if ((SArea[sq + Push(me)] & King(opp) & Line(me, 7)) != Empty)
				return 0;
			if ((SArea[sq + Push(me)] & SArea[lsb(King(opp))] & Line(me, 7)) != Empty &&
			    ((NAtt[sq + Push(me)] & Knight(me)) == Empty || turn == opp))
				return 0;
		}
	}
	return 32;
...
template <bool me> int krpkrx(const int turn, const uint64_t att[], const GBoard *Board)
{
	...
	int o_rook = lsb(Rook(opp));
	int m_king = lsb(King(me));
	int add_mat = (Piece(opp) ^ King(opp) ^ Rook(opp)) != Empty;
	int clear = !(add_mat) || ((PWay[opp][sq] | PIsolated[file_of(sq)]) & Forward[opp][rank_of(sq + Push(me))] &
	                           (Piece(opp) ^ King(opp) ^ Rook(opp))) == Empty;

	if (!clear)
		return 32;
	if (!add_mat && (Pawn(me) & (File[0] | File[7])) == Empty) {
		int m_rook = lsb(Rook(me));
		if (CRank(me, o_king) < CRank(me, m_rook) && CRank(me, m_rook) < rrank && CRank(me, m_king) >= rrank - 1 &&
		    CRank(me, m_king) > CRank(me, m_rook) &&
		    ((SArea[m_king] & Pawn(me)) != Empty ||
		     (turn == me && std::abs(file_of(sq) - file_of(m_king)) <= 1 && std::abs(rrank - CRank(me, m_king)) <= 2)))
			return 128;
		if ((SArea[m_king] & Pawn(me)) != Empty) {
			if (rrank >= 4) {
				if ((file_of(sq) < file_of(m_rook) && file_of(m_rook) < file_of(o_king)) ||
				    (file_of(sq) > file_of(m_rook) && file_of(m_rook) > file_of(o_king)))
					return 128;
			} else if (rrank >= 2) {
				if ((Pawn(me) & (File[1] | File[6])) == Empty && rrank + std::abs(file_of(sq) - file_of(m_rook)) > 4 &&
				    ((file_of(sq) < file_of(m_rook) && file_of(m_rook) < file_of(o_king)) ||
				     (file_of(sq) > file_of(m_rook) && file_of(m_rook) > file_of(o_king))))
					return 128;
			}
	...
	if ((PWay[me][sq] & King(opp)) != Empty) {
		...
		if (rrank == 4 && CRank(me, m_king) <= 4 && CRank(me, o_rook) == 5 &&
		    (King(opp) & (Line(me, 6) | Line(me, 7))) != Empty &&
		    (turn != me || !(PAtt[me][sq] & RookAttacks(lsb(Rook(me)), PieceAll) & (~SArea[o_king])) == Empty))
			mul = std::min(mul, add_mat << 3);
		if (rrank >= 5 && CRank(me, o_rook) <= 1 && (turn != me || AtCheck(me) || Dist(m_king, sq) >= 2))
			mul = std::min(mul, add_mat << 3);
		if ((King(opp) & (File[1] | File[2] | File[6] | File[7])) != Empty && (Rook(opp) & Line(me, 7)) != Empty &&
		    (Between[o_king][o_rook] & (File[3] | File[4])) != Empty && !(Rook(me) & Line(me, 7)))
			mul = std::min(mul, add_mat << 3);
		return mul;
	} else if (rrank == 6 && (Pawn(me) & (File[0] | File[7])) != Empty &&
	           ((PSupport[me][sq] | PWay[opp][sq]) & Rook(opp)) != Empty && CRank(me, o_king) >= 6) {
		int dist = std::abs(file_of(sq) - file_of(o_king));
		if (dist <= 3)
			mul = std::min(mul, add_mat << 3);
		if (dist == 4 && ((PSupport[me][o_king] & Rook(me)) != Empty || turn == opp))
			mul = std::min(mul, add_mat << 3);
	}
	...
	if (((PWay[me][sq] & Rook(me)) != Empty) && ((PWay[opp][sq] & Rook(opp))) != Empty) {
		if ((King(opp) & (File[0] | File[1] | File[6] | File[7]) & Line(me, 6)) != Empty)
			mul = std::min(mul, add_mat << 3);
		else if ((Pawn(me) & (File[0] | File[7])) != Empty && (King(opp) & (Line(me, 5) | Line(me, 6))) != Empty &&
	...
	if (std::abs(file_of(sq) - file_of(o_king)) <= 1 && std::abs(file_of(sq) - file_of(o_rook)) <= 1 &&
		...
		mul = std::min(mul, (Pawn(me) & (File[3] | File[4])) != Empty ? 12 : 16);
...
template <bool me> int krpkbx(const int turn, const GBoard *Board)
{
	...
	if (CRank(me, diag_sq) > 1)
		return 32;
	bitboard_t mdiag = FullLine[sq + Push(me)][diag_sq] | Bit(sq + Push(me)) | Bit(diag_sq);
	int check_sq = NB(me, BMask[sq - Push(me)]);
	bitboard_t cdiag = FullLine[sq - Push(me)][check_sq] | Bit(sq - Push(me)) | Bit(check_sq);
	if (((mdiag | cdiag) & (Piece(opp) ^ King(opp) ^ Bishop(opp))) != Empty)
		return 32;
	if ((cdiag & Bishop(opp)) != Empty)
		return 0;
	if ((mdiag & Bishop(opp)) != Empty && (turn == opp || (King(me) & PAtt[opp][sq + Push(me)]) == Empty))
		return 0;
	return 32;
}
...
{
	constexpr bool opp = !me;
	if ((SArea[lsb(King(opp))] & Pawn(opp) & Line(me, 1) & (File[0] | File[2] | File[5] | File[7])) == Empty)
		return 32;
	if ((PWay[opp][lsb(Pawn(opp))] & (King(me) | Queen(me))) != Empty)
		return 32;
	if ((Pawn(opp) & (File[0] | File[7])) != Empty)
		return 1;
	else
		return 4;
}
template <bool me> int kqkrpx(const GBoard *Board)
{
	constexpr bool opp = !me;
	int rsq = lsb(Rook(opp));
	bitboard_t pawns = SArea[lsb(King(opp))] & PAtt[me][rsq] & Pawn(opp) & Interior & Line(me, 6);
	if (pawns != Empty && CRank(me, lsb(King(me))) <= 4)
		return 0;
	return 32;
}
...
template <bool me> int krpppkrppx(const bitboard_t passer, const GBoard *Board)
{
	constexpr bool opp = !me;
...
template <bool me> int kbpkbx(const int turn, const GBoard *Board)
{
	...
	if (((Board->bb[ILight(me)] != Empty) && (Board->bb[IDark(opp)] != Empty)) ||
		...
		if (((PWay[me][sq] & King(opp)) != 0) && CRank(me, sq) <= 5)
			return 0;
		for (u = Bishop(opp); u != Empty; Cut(u)) {
			if (CRank(me, lsb(u)) <= 4 && ((BishopAttacks(lsb(u), PieceAll) & PWay[me][sq]) != 0))
				return 0;
			if (turn == opp && ((BishopAttacks(lsb(u), PieceAll) & Pawn(me)) != 0))
				return 0;
		}
...
template <bool me> int kbppkbx(const GBoard *Board)
{
	...
	if (file_of(sq1) == file_of(sq2)) {
		if (CRank(me, sq2) <= 3)
			return 0;
		if (((PWay[me][sq2] & King(opp)) != Empty) && CRank(me, sq2) <= 5)
			return 0;
	} else if (PIsolated[file_of(sq1)] & Pawn(me)) {
		if (((King(opp) & LightArea) != Empty) != ((Bishop(me) & LightArea) != Empty)) {
			if ((((SArea[o_king] | King(opp)) & Bit(sq2 + Push(me))) != Empty) &&
			    ((BishopAttacks(o_bishop, PieceAll) & Bit(sq2 + Push(me))) != Empty))
				if ((((SArea[o_king] | King(opp)) & Bit((sq2 & 0xFFFFFFF8) | file_of(sq1))) != Empty) &&
				    ((BishopAttacks(o_bishop, PieceAll) & Bit((sq2 & 0xFFFFFFF8) | file_of(sq1))) != Empty))
					return 0;
		}
	}
	return 32;
...
template <bool me, bool HPopCnt> void Position::eval_queens(GEvalInfo &EI)
{
	...
	for (u = Queen(me); u != 0; u ^= b) {
		...
		if (QMask[sq] & King(opp))
			if (uint64_t v = Between[PVarC(EI, king, opp)][sq] & EI.occ)
				if (Single(v)) {
					Current->xray[me] |= v;
					uint64_t square = lsb(v);
					...
					if (piece == IPawn(me)) {
						if (!Square(square + Push(me)))
							IncV(EI.score, Ca(Pin, SelfPawnPin));
					} else if ((piece & 1) == me) {
						IncV(EI.score, Ca(Pin, SelfPiecePin));
						katt = 1;
					} else if (piece != IPawn(opp) &&
					...
					IncV(EI.score, Ca(KingRay, QKingRay));
		if (att & PVarC(EI, area, opp)) {
			PVarC(EI, king_att, me) += KingQAttack;
			for (uint64_t v = att & PVarC(EI, area, opp); v != 0; Cut(v))
		...
		if (att & PVarC(EI, free, me) & Pawn(opp))
			IncV(EI.score, Ca(Tactical, TacticalMajorPawn));
		if (att & PVarC(EI, free, me) & Minor(opp))
			IncV(EI.score, Ca(Tactical, TacticalMajorMinor));
...
template <bool me, bool HPopCnt> void Position::eval_rooks(GEvalInfo &EI)
{
	...
	for (u = Rook(me); u != 0; u ^= b) {
				...
				if (Single(v)) {
					...
					if (piece == IPawn(me)) {
						...
						if (piece < IRook(opp)) {
							...
							IncV(EI.score, Ca(Pin, ThreatPin));
					}
					if (katt && !(att & PVarC(EI, area, opp)))
						PVarC(EI, king_att, me) += KingAttack;
				} else if (v == (v & (Minor(opp) | Queen(opp))))
					IncV(EI.score, Ca(KingRay, RKingRay));
		if (att & PVarC(EI, area, opp)) {
			PVarC(EI, king_att, me) += KingRAttack;
			for (uint64_t v = att & PVarC(EI, area, opp); v != 0; Cut(v))
		...
		if (att & PVarC(EI, free, me) & Minor(opp))
			IncV(EI.score, Ca(Tactical, TacticalMajorMinor));
		if (att & PVarC(EI, area, me))
			IncV(EI.score, Ca(KingDefence, KingDefRook));
		Current->threat |= att & Queen(opp);
		if (!(PWay[me][sq] & Pawn(me))) {
			IncV(EI.score, Ca(RookSpecial, RookHof));
			int hof_score = 0;
			if (!(PWay[me][sq] & Pawn(opp))) {
				IncV(EI.score, Ca(RookSpecial, RookOf));
				if (att & Line(me, 7))
					hof_score += Ca(RookSpecial, RookOfOpen);
				else if (uint64_t target = att & PWay[me][sq] & Minor(opp)) {
					if (!(Current->patt[opp] & target)) {
						hof_score += Ca(RookSpecial, RookOfMinorHaging);
						if (PWay[me][sq] & King(opp))
							hof_score += Ca(RookSpecial, RookOfKingAtt);
					} else
						hof_score += Ca(RookSpecial, RookOfMinorFixed);
				}
			} else if (att & PWay[me][sq] & Pawn(opp)) {
				uint64_t square = lsb(att & PWay[me][sq] & Pawn(opp));
				if (!(PSupport[opp][square] & Pawn(opp)))
					hof_score += Ca(RookSpecial, RookHofWeakPAtt);
			}
			IncV(EI.score, hof_score);
		...
		if ((b & Line(me, 6)) && ((King(opp) | Pawn(opp)) & (Line(me, 6) | Line(me, 7)))) {
			...
			if (King(opp) & Line(me, 7))
				IncV(EI.score, Ca(RookSpecial, Rook7thK8th));
		...
		if (BMask[sq] & King(opp))
			if (uint64_t v = Between[PVarC(EI, king, opp)][sq] & EI.occ)
				if (Single(v)) {
					...
					IncV(EI.score, Ca(KingRay, BKingRay));
		if (att & PVarC(EI, area, opp))
			PVarC(EI, king_att, me) += KingBAttack;
		IncV(EI.score, Mobility[PieceType[WhiteLight] - 1][popcount<HPopCnt>(att & PVarC(EI, free, me))]);
		if (att & PVarC(EI, free, me) & Pawn(opp))
			IncV(EI.score, Ca(Tactical, TacticalMinorPawn));
		if (att & PVarC(EI, free, me) & Knight(opp))
			IncV(EI.score, Ca(Tactical, TacticalMinorMinor));
		if (att & PVarC(EI, area, me))
			IncV(EI.score, Ca(KingDefence, KingDefBishop));
		Current->threat |= att & Major(opp);
		if (b & LightArea) {
			for (uint64_t v = ((~BishopForward[me][sq]) | (att & Forward[me][rank_of(sq)])) & Pawn(opp) &
			                  (~Current->patt[opp]) & LightArea;
			     v; Cut(v)) {
				uint64_t square = lsb(v);
				if (!((PSupport[opp][square] | PWay[opp][square]) & Pawn(opp)))
					IncV(EI.score, Ca(BishopSpecial, BishopNonForwardPawn));
			}
			uint64_t v = BishopForward[me][sq] & Pawn(me) & LightArea;
			v |= (v & (File[2] | File[3] | File[4] | File[5] | BMask[sq])) >> 8;
			DecV(EI.score, Ca(BishopSpecial, BishopPawnBlock) * popcount<HPopCnt>(v));
...
template <bool me, bool HPopCnt> void Position::eval_knights(GEvalInfo &EI)
{
	...
	for (u = Knight(me); u != 0; u ^= b) {
		...
		if (att & PVarC(EI, free, me) & Bishop(opp))
			IncV(EI.score, Ca(Tactical, TacticalMinorMinor));
		if (att & PVarC(EI, area, me))
			IncV(EI.score, Ca(KingDefence, KingDefKnight));
		Current->threat |= att & Major(opp);
		if ((b & Outpost[me]) && !(Pawn(opp) & PIsolated[file_of(sq)] & Forward[me][rank_of(sq)])) {
			IncV(EI.score, Ca(KnightSpecial, KnightOutpost));
			if (Current->patt[me] & b) {
				IncV(EI.score, Ca(KnightSpecial, KnightOutpostProtected));
				if (att & PVarC(EI, free, me) & Pawn(opp))
					IncV(EI.score, Ca(KnightSpecial, KnightOutpostPawnAtt));
				if (att & PVarC(EI, free, me) & Bishop(opp))
					IncV(EI.score, Ca(KnightSpecial, KnightOutpostBishopAtt));
			}
		}
	}
}
template <bool me, bool HPopCnt> void Position::eval_king(GEvalInfo &EI)
{
	...
	int score = Endgame(PVarC(EI, king_att, me));
	if (cnt >= 2 && (Queen(me) != 0)) {
		if (uint64_t u = Current->att[me] & PVarC(EI, area, opp) & (~Current->att[opp]))
...
template <bool me, bool HPopCnt> void Position::eval_endgame(GEvalInfo &EI)
{
	...
	if ((EI.material->flags & VarC(FlagSingleBishop, me)) && Pawn(me)) {
		int sq = (Board->bb[ILight(me)]
		              ? (me ? 0 : 63)
		              : (Board->bb[IDark(me)] ? (me ? 7 : 56)
		                                      : (file_of(lsb(King(opp))) <= 3 ? (me ? 0 : 56) : (me ? 7 : 63))));
		if (!(Pawn(me) & (~PWay[opp][sq]))) {
			if ((SArea[sq] | Bit(sq)) & King(opp))
				...
				EI.mul = 0;
		} else if ((King(opp) & Line(me, 6) | Line(me, 7)) && std::abs(file_of(sq) - file_of(lsb(King(opp)))) <= 3 &&
		...
		check_for_partial_block:
			...
			if ((SArea[PVarC(EI, king, opp)] | Current->patt[opp]) & Bishop(opp)) {
				...
				if (!(push & (~(Piece(opp) | Current->att[opp]))) &&
					...
					int bsq = lsb(Bishop(opp));
					uint64_t att = BishopAttacks(bsq, EI.occ);
					uint64_t prp = (att | SArea[PVarC(EI, king, opp)]) & Pawn(opp) &
					               (Board->bb[ILight(opp)] ? LightArea : DarkArea);
					uint64_t patt = ShiftW(opp, prp) | ShiftE(opp, prp);
					if ((SArea[PVarC(EI, king, opp)] | patt) & Bishop(opp)) {
						uint64_t double_att =
	...
	if (!(Major(me))) {
		if ((Bishop(me) != 0) && !(Knight(me)) && Single(Bishop(me)) && (Pawn(me) != 0)) {
			...
			if (number == 1) {
				if (Bishop(opp))
					EI.mul = MinF(EI.mul, kbpkbx<me>(Current->turn, Board));
				else if (Knight(opp))
					EI.mul = MinF(EI.mul, kbpknx<me>(Current->turn, Board));
			} else if (number == 2 && (Bishop(opp) != 0))
				EI.mul = MinF(EI.mul, kbppkbx<me>(Board));
		} else if (!Bishop(me) && Knight(me) && Single(Knight(me)) && Pawn(me) && Single(Pawn(me)))
			EI.mul = MinF(EI.mul, knpkx<me>(Current->turn, Board));
	} else if (!(Minor(me))) {
		if (!(Pawn(me)) && !(Rook(me)) && (Queen(me) != 0) && (Pawn(opp) != 0)) {
			if (!(NonPawnKing(opp)) && Single(Pawn(opp)))
				EI.mul = MinF(EI.mul, kqkp<me>(Board));
			else if (Rook(opp))
				EI.mul = MinF(EI.mul, kqkrpx<me>(Board));
		} else if (!(Queen(me)) && (Rook(me) != 0) && Single(Rook(me))) {
			int number = popcount<HPopCnt>(Pawn(me));
			if (number <= 3) {
				if (number == 0) {
					if (Pawn(opp))
						EI.mul = MinF(EI.mul, krkpx<me>(Board));
				} else if (Rook(opp)) {
					if (number == 1) {
						...
						if (number == 2)
							EI.mul = MinF(EI.mul, krppkrx<me>(Board));
						if (Pawn(opp)) {
							if (number == 2)
								EI.mul = MinF(EI.mul, krppkrpx<me>(Current->passer, Board));
							else if (Multiple(Pawn(opp)))
								EI.mul = MinF(EI.mul, krpppkrppx<me>(Current->passer, Board));
						}
					}
				} else if (number == 1 && Bishop(opp))
					EI.mul = MinF(EI.mul, krpkbx<me>(Current->turn, Board));
			}
		}
	} else if (!Pawn(me) && Single(Rook(me)) && !Queen(me) && Single(Bishop(me)) && !Knight(me) && Rook(opp))
		EI.mul = MinF(EI.mul, krbkrx<me>(Board));
	if (!(NonPawnKing(opp)) && Current->turn == opp && !(Current->att[me] & King(opp)) &&
...
template <bool HPopCnt> void Position::eval_unusual_material(const int turn, GEvalInfo &EI)
{
	...
	bq = popcount<HPopCnt>(Queen(Black));
	int phase = std::min(24, (wlight + blight) + 2 * (wr + br) + 4 * (wq + bq));
	int mat_score = SeeValue[WhitePawn] * (wp - bp) + SeeValue[WhiteKnight] * (wlight - blight) +
	                SeeValue[WhiteRook] * (wr - br) + SeeValue[WhiteQueen] * (wq - bq);
	mat_score = Compose(mat_score, mat_score);
	Current->score = (((Opening(mat_score + EI.score) * phase) + (Endgame(mat_score + EI.score) * (24 - phase))) / 24);
	if (turn)
...
#undef opp
	...
	Current->score =
	    EI.material->score +
	    (((Opening(EI.score) * EI.material->phase) + (Endgame(EI.score) * (128 - (int)EI.material->phase))) / 128);
	...
	if (Current->score > 0) {
		...
		EI.mul = std::min(EI.material->mul[White], EI.material->mul[Black]);
	Current->score = (Current->score * EI.mul) / 32;
