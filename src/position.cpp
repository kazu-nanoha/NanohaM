...
template <bool me> void Position::do_move(int move)
{
	...
	if (piece == IPawn(me)) {
		if (IsPromotion(move)) {
			...
			if (piece < WhiteRook) {
				if (piece >= WhiteLight && BB(piece) != Empty)
					Next->material |= FlagUnusualMaterial;
...
template <bool me> int Position::is_legal(int move)
{
	...
	if (piece >= WhiteLight && piece < WhiteKing) {
		if ((QMask[from] & u) == Empty)
			return 0;
		if ((Between[from][to] & occ) != Empty)
			return 0;
	}
	if (IsPromotion(move) && Board->square[from] >= WhiteKnight)
		return 0;
	if (piece == IPawn(me)) {
		if ((u & PMove[me][from]) != Empty) {
			if (capture)
				return 0;
			if ((u & Line(me, 7)) != Empty && !IsPromotion(move))
				return 0;
			return 1;
			...
			if (Square(to - Push(me)))
				return 0;
			if ((u & Line(me, 3)) != Empty)
				return 0;
			return 1;
		} else if ((u & PAtt[me][from]) != Empty) {
			if (capture == 0)
				return 0;
			if ((u & Line(me, 7)) != Empty && !IsPromotion(move))
				return 0;
		...
		if ((SArea[from] & u) != Empty)
			return 0;
		if ((Current->att[opp] & u) != Empty)
			return 0;
		return 1;
	}
	piece = (piece >> 1) - 2;
	if (piece == 0) {
		if ((u & NAtt[from]) != Empty)
			return 1;
		else
			return 0;
	} else {
		if (piece <= 2) {
			if ((BMask[from] & u) != Empty)
				return 1;
		} else if (piece == 3) {
			if ((RMask[from] & u) != Empty)
				return 1;
		} else
			return 1;
		return 0;
...
{ // doesn't detect castling and ep checks
	...
	king_sq = lsb(king);
	piece = Square(from);
	if ((Bit(from) & Current->xray[me]) != Empty && (FullLine[king_sq][from] & Bit(to)) != Empty)
		return 1;
	if (piece < WhiteKnight) {
		if ((PAtt[me][to] & king) != Empty)
			return 1;
		if ((Bit(to) & Line(me, 7)) != Empty && (king & Line(me, 7)) != Empty &&
		    (Between[to][king_sq] & PieceAll) != Empty)
			return 1;
	} else if (piece < WhiteLight) {
		if ((NAtt[to] & king) != Empty)
			return 1;
	} else if (piece < WhiteRook) {
		if ((BMask[to] & king) != Empty)
			if ((Between[king_sq][to] & PieceAll) != Empty)
				return 1;
	} else if (piece < WhiteQueen) {
		if ((RMask[to] & king) != Empty)
			if ((Between[king_sq][to] & PieceAll) != Empty)
				return 1;
	} else if (piece < WhiteKing) {
		if ((QMask[to] & king) != Empty)
			if ((Between[king_sq][to] & PieceAll) != Empty)
				return 1;
	}
	return 0;
...
template <bool me> int Position::see(int move, int margin)
{
	...
	bitboard_t clear, def, att, occ, b_area, r_slider_att, b_slider_att;
	bitboard_t r_slider_def, b_slider_def, r_area, u, new_att, my_bishop, opp_bishop;
	from = From(move);
	to = To(move);
	...
	if (piece == SeeValue[WhiteKing])
		return 1;
	if ((Current->xray[me] & Bit(from)) != Empty)
		return 1;
	if ((Current->pin[me] & Bit(from)) != Empty && piece <= SeeValue[WhiteDark])
		return 1;
	if (piece > (SeeValue[WhiteKing] >> 1))
		return 1;
	if ((Current->att[opp] & Bit(to)) != Empty)
		return 1;
	att = PAtt[me][to] & Pawn(opp);
	if (att != Empty && delta + margin > SeeValue[WhitePawn])
		return 0;
	clear = ~Bit(from);
	def = PAtt[opp][to] & Pawn(me) & clear;
	if (def != Empty && delta + SeeValue[WhitePawn] + margin <= 0)
		return 1;
	att |= NAtt[to] & Knight(opp);
	if (att != Empty && delta > SeeValue[WhiteDark] - margin)
		return 0;
	occ = PieceAll & clear;
	b_area = BishopAttacks(to, occ);
	opp_bishop = Bishop(opp);
	if (delta > SeeValue[IDark(me)] - margin)
		if ((b_area & opp_bishop) != Empty)
			return 0;
	my_bishop = Bishop(me);
	...
	while (true) {
		if ((u = (att & Pawn(opp))) != Empty) {
			capture -= piece;
			piece = SeeValue[WhitePawn];
			sq = lsb(u);
			occ ^= Bit(sq);
			att ^= Bit(sq);
			for (new_att = FullLine[to][sq] & b_slider_att & occ & (~att); new_att != Empty; Cut(new_att)) {
				pos = lsb(new_att);
				if ((Between[to][pos] & occ) != Empty) {
					Add(att, pos);
					break;
				}
			}
		} else if ((u = (att & Knight(opp))) != Empty) {
			capture -= piece;
			piece = SeeValue[WhiteKnight];
			// att ^= (~(u - 1)) & u;
			Cut(att);
		} else if ((u = (att & opp_bishop)) != Empty) {
			capture -= piece;
		...
		} else if ((u = (att & Rook(opp))) != Empty) {
			capture -= piece;
			piece = SeeValue[WhiteRook];
			sq = lsb(u);
			occ ^= Bit(sq);
			att ^= Bit(sq);
			for (new_att = FullLine[to][sq] & r_slider_att & occ & (~att); new_att != Empty; Cut(new_att)) {
				pos = lsb(new_att);
				if ((Between[to][pos] & occ) != Empty) {
					Add(att, pos);
					break;
		...
		} else if ((u = (att & Queen(opp))) != Empty) {
			capture -= piece;
			piece = SeeValue[WhiteQueen];
			sq = lsb(u);
			occ ^= Bit(sq);
			att ^= Bit(sq);
			for (new_att = FullLine[to][sq] & (r_slider_att | b_slider_att) & occ & (~att); new_att != Empty;
			     Cut(new_att)) {
				pos = lsb(new_att);
				if ((Between[to][pos] & occ) != Empty) {
					Add(att, pos);
		...
		if (piece + capture < margin)
			return 0;
		if ((u = (def & Pawn(me))) != Empty) {
			capture += piece;
			piece = SeeValue[WhitePawn];
			sq = lsb(u);
			occ ^= Bit(sq);
			def ^= Bit(sq);
			for (new_att = FullLine[to][sq] & b_slider_def & occ & (~att); new_att != Empty; Cut(new_att)) {
				pos = lsb(new_att);
				if (testz_bb(Between[to][pos], occ)) {
					Add(def, pos);
					break;
				}
			}
		} else if ((u = (def & Knight(me))) != Empty) {
			capture += piece;
			piece = SeeValue[WhiteKnight];
			// def ^= (~(u - 1)) & u;
			Cut(def);
		} else if ((u = (def & my_bishop)) != Empty) {
			capture += piece;
			piece = SeeValue[WhiteDark];
			sq = lsb(u);
			occ ^= Bit(sq);
		...
		} else if ((u = (def & Rook(me))) != Empty) {
			capture += piece;
			piece = SeeValue[WhiteRook];
			sq = lsb(u);
			occ ^= Bit(sq);
			def ^= Bit(sq);
			for (new_att = FullLine[to][sq] & r_slider_def & occ & (~att); new_att != Empty; Cut(new_att)) {
				pos = lsb(new_att);
				if (!testz_bb(Between[to][pos], occ)) {
					Add(def, pos);
					break;
		...
		} else if ((u = (def & Queen(me))) != Empty) {
			capture += piece;
			piece = SeeValue[WhiteQueen];
			sq = lsb(u);
			occ ^= Bit(sq);
			def ^= Bit(sq);
			for (new_att = FullLine[to][sq] & (r_slider_def | b_slider_def) & occ & (~att); new_att != Empty;
			     Cut(new_att)) {
				pos = lsb(new_att);
				if (testz_bb(Between[to][pos], occ)) {
					Add(def, pos);
